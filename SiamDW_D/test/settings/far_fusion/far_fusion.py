from libs.utils import TrackerParams, FeatureParams, Choice
from libs.features.extractor import MultiResolutionExtractor
from libs.features import deep
import torch
import os
...
def parameters():
    ...
    params.long_term = True

    params.read_online = True
    # params.main_path = '$CODE-PATH/SiamDW_D/far/'
    params.main_path = '/data/home/v-hongyuan/projects/github/lt_tracking/test/'
    params.read_filter_path = osp.join(params.main_path, "networks/online_models/filter.pth")
    params.read_proj_path = osp.join(params.main_path, "networks/online_models/proj.pth")
    ...
    # params.sample_memory_size = 450  # Memory size
    params.train_skipping = 10  # How often to run training (every n-th frame)
    # params.train_skipping = 15  # How often to run training (every n-th frame)
    ...
    params.update_scale_when_uncertain = False  # Update scale or not if distractor is close
    ...
    deep_feat = deep.ATOMSENet154(net_path=osp.join(params.main_path, 'networks/main/senet154_TLVC.pth.tar'), output_layers=['layer3'], fparams=deep_fparams,
                                  normalize_power=2)
