...
class BaseTracker:
    ...
    def track_rgbd(self, raw_im, image, depth_im, gt):
        """Overload this function in your tracker. This should track in the frame and update the model."""
        raise NotImplementedError
    ...
    def track_sequence_rgbd(self, sequence):
        ...
        for frame in sequence.frames[1:]:
            ...
            # import pdb; pdb.set_trace()
            state, score = self.track_rgbd(raw_im, image, depth_im, [cx, cy, w, h])
            ...
            # print ("for frame: {} iou: {:.3f}".format(osp.basename(frame), iou))
            ...
            cnt += 1
        print("for video: {} mean iou: {}".format(sequence.name, np.mean(all_ious)))
