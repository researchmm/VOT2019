...
from libs.models.modules.blocks import LinearBlock
from libs.PreciseRoIPooling.pytorch.prroi_pool import PrRoIPool2D
...
class IoUNet_dropout(nn.Module):
    """ Network module for IoU prediction. Refer to the paper for an illustration of the architecture."""
    def __init__(self, input_dim=(128,256), pred_input_dim=(256,256), pred_mid_dim=(128,256), pred_inter_dim=(256,256)):
        super().__init__()
        # _r for reference, _t for test
        self.conv3_1r = conv(input_dim[0], pred_mid_dim[0], kernel_size=3, stride=1)
        self.conv3_1t = conv(input_dim[0], pred_mid_dim[1], kernel_size=3, stride=1)

        self.conv3_2t = conv(pred_mid_dim[1], pred_input_dim[0], kernel_size=3, stride=1)

        self.prroi_pool3r = PrRoIPool2D(3, 3, 1/8)
        self.prroi_pool3t = PrRoIPool2D(5, 5, 1/8)

        self.fc3_1r = conv(pred_mid_dim[0], pred_mid_dim[1], kernel_size=3, stride=1, padding=0)

        self.conv4_1r = conv(input_dim[1], pred_mid_dim[1], kernel_size=3, stride=1)
        self.conv4_1t = conv(input_dim[1], pred_mid_dim[1], kernel_size=3, stride=1)

        self.conv4_2t = conv(pred_mid_dim[1], pred_input_dim[1], kernel_size=3, stride=1)

        self.prroi_pool4r = PrRoIPool2D(1, 1, 1 / 16)
        self.prroi_pool4t = PrRoIPool2D(3, 3, 1 / 16)

        self.fc34_3r = conv(pred_mid_dim[1] + pred_mid_dim[1], pred_input_dim[0], kernel_size=1, stride=1, padding=0)
        self.fc34_4r = conv(pred_mid_dim[1] + pred_mid_dim[1], pred_input_dim[1], kernel_size=1, stride=1, padding=0)
