...
from libs.utils import complex, fourier
...
def hann1d(sz: int, centered = True) -> torch.Tensor:
    ...
    return torch.cat([w, flip(w[1:sz-sz//2], 0)])


def hann2d(sz: torch.Tensor, centered = True) -> torch.Tensor:
    """2D cosine window."""
    return hann1d(sz[0], centered).view(1, 1, -1, 1) * hann1d(sz[1], centered).view(1, 1, 1, -1)
...
def label_function(sz: torch.Tensor, sigma: torch.Tensor):
    return gauss_fourier(sz[0], sigma[0]).reshape(1, 1, -1, 1) * gauss_fourier(sz[1], sigma[1], True).reshape(1, 1, 1, -1)
...
def label_function_spatial(sz: torch.Tensor, sigma: torch.Tensor, center: torch.Tensor = torch.zeros(2), end_pad: torch.Tensor = torch.zeros(2)):
    ...
    return gauss_spatial(sz[0], sigma[0], center[0], end_pad[0]).view(1, 1, -1, 1) * \
           gauss_spatial(sz[1], sigma[1], center[1], end_pad[1]).view(1, 1, 1, -1)
...
def max2d(a: torch.Tensor) -> (torch.Tensor, torch.Tensor):
    ...
    argmax_row = argmax_row.view(argmax_col.numel(),-1)[torch.arange(argmax_col.numel()).int().tolist(), argmax_col.view(-1).tolist()]
    argmax_row = argmax_row.view(argmax_col.shape)
