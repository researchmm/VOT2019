...
from libs.utils import complex, TensorList
from libs.utils.tensorlist import tensor_operation
...
def cfft2(a):
    ...
    turple = fft.rfft2(a)
    rst = torch.cat([turple[0].unsqueeze(-1), turple[1].unsqueeze(-1)], dim=-1)
    return rfftshift2(rst)
...
def cifft2(a, signal_sizes=None):
    ...
    rst = irfftshift2(a)
    rst = torch.split(rst, split_size=1, dim=-1)
    return fft.irfft2(rst[0].squeeze(-1).contiguous(), rst[1].squeeze(-1).contiguous())
...
def sample_fs(a: torch.Tensor, grid_sz: torch.Tensor = None, rescale = True):
    ...
    if rescale:
        return grid_sz.prod() * cifft2(F.pad(a, (0, 0, 0, pad_right, pad_top, pad_bottom)).data, signal_sizes=grid_sz.long().tolist())
    else:
...
def get_frequency_coord(sz, add_complex_dim = False, device='cpu'):
    ...
    ky = torch.arange(-int((sz[0]-1)/2), int(sz[0]/2+1)).cuda().view(1,1,-1,1)
    kx = torch.arange(0, int(sz[1]/2+1)).float().cuda().view(1,1,1,-1)
