...
# from FPNlib.mmdet.datasets import to_tensor
from libs.FPNlib.mmdet.datasets.transforms import ImageTransform
...
def _prepare_data(img, img_transform, cfg, device):
    ...
    img = Variable(torch.from_numpy(img)).cuda().unsqueeze(0)
...
def _inference_single(model, img, img_transform, cfg, device):
    ...
    # with torch.no_grad():
    result, f_list = model(return_loss=False, rescale=True, **data)
