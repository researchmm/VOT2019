...
from libs.utils.interpolate import interpolate
from libs.features.preprocessing import numpy_to_torch, torch_to_numpy
...
class FlipHorizontal(Transform):
    ...
    def __call__(self, image):
        if isinstance(image, torch.Tensor):
            temp = np.flip(image.numpy(), 3).copy()
...
class FlipVertical(Transform):
    ...
    def __call__(self, image: torch.Tensor):
        if isinstance(image, torch.Tensor):
            temp = np.flip(image.numpy(), 2).copy()
            image = torch.from_numpy(temp)
            return self.crop_to_output(image)
        else:
...
class Scale(Transform):
    ...
    def __call__(self, image):
        if isinstance(image, torch.Tensor):
            ...
            image_resized = interpolate(image, [h_new, w_new], mode='bilinear')
...
class Blur(Transform):
    ...
    def __init__(self, sigma, output_sz = None, shift = None):
        ...
        x_coord = [torch.arange(-sz, sz+1) for sz in self.filter_size]
    ...
    def __call__(self, image):
        if isinstance(image, torch.Tensor):
            sz = image.shape[2:]
            im1 = F.conv2d(Variable(image.view(-1,1,sz[0],sz[1])), Variable(self.filter[0]), padding=(self.filter_size[0],0))
            res = F.conv2d(im1, Variable(self.filter[1]), padding=(0,self.filter_size[1])).view(1,-1,sz[0],sz[1])
            return self.crop_to_output(res.data)
        else:
            raise NotImplementedError