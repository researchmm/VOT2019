...
class FAR_FUSION(BaseTracker):
    ...
    def initialize(self, raw_im, image, depth_im, state, *args, **kwargs):
        #### add tracker senet ####
        ...
        self.tracker_senet.initialize(raw_im, image, depth_im, state, *args, **kwargs)
        ...
        self.tracker_resnext.initialize(raw_im, image, depth_im, state, *args, **kwargs)

        #### add tracker senet flip ####
        #tracker_senet_flip = 'senet_far'
        #param_senet_flip = 'senet'
        #senet_flip_params = self.get_tracker_para(tracker_senet_flip, param_senet_flip)
        #self.tracker_senet_flip = SenetFar(senet_flip_params)
        #raw_im_flip, image_flip, depth_flip = cv2.flip(raw_im.copy(), 1), cv2.flip(image.copy(), 1), cv2.flip(depth_im.copy(), 1)
        #self.imw = raw_im.shape[1]
        #state_flip = state.copy()
        #state_flip[0] = self.imw - state[0]
        #self.tracker_senet_flip.initialize(raw_im_flip, image_flip, depth_flip, state_flip, *args, **kwargs)
    ...
    def get_tracker_para(self, tracker, name):
        param_module = importlib.import_module('far.parameter.{}.{}'.format(tracker, name))
