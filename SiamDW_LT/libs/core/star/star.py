...
from libs.utils import operation, dcf, fourier, TensorList
...
class STAR(BaseTracker):
    ...
    def initialize(self, raw_im, image, state, *args, **kwargs):
        ...
        model = ModelBuilder()
        path = local_env_settings().network_path
        path = os.path.join(path, "res50_pysot.pth")
    ...
    def track(self, raw_im, image, gt):
        ...
        score, bbox = self.SiamRPN_track(raw_im.copy())
        if score < 0.8:
        ...
        # ATOM first analyse
        self.im = numpy_to_torch(image)
        sample_pos = self.pos.round()
        sample_scales = self.target_scale * self.params.scale_factors
        test_x = self.extract_processed_sample(self.im.clone(), sample_pos, sample_scales, self.img_sample_sz)
        scores_raw = self.apply_filter(test_x)
        translation_vec, scale_ind, s, flag = self.localize_target(scores_raw)
        mx_score = torch.max(s[int(scale_ind), ...])

        RPN_score, ATOM_score = score, mx_score

        # both absent ---> return RPN box
        if mx_score < getattr(self.params, 'athr_low', 0.5) and RPN_score < 0.8:
            ...
            if mx_iou > getattr(self.params, 'iou_thr', 0.1):
                ...
                self.center_pos = np.array([box[0], box[1]])
            ...
            return [target_pos[0] - target_sz[0] / 2.,
                    target_pos[1] - target_sz[1] / 2.,
                    target_sz[0], target_sz[1]], mx_score
        elif mx_score > getattr(self.params, 'athr_high', 0.5) and RPN_score > 0.998:
            self.update_state(sample_pos + translation_vec)
            self.refine_target_box(sample_pos, sample_scales[int(scale_ind)], int(scale_ind), True)

            hard_negative = (flag == 'hard_negative')
            learning_rate = self.params.hard_negative_learning_rate if hard_negative else None

            if float(self.predicted_iou) > 0.7:
            ...
            return new_state.tolist(), mx_score
        else:
            if mx_score > getattr(self.params, 'athr_low', 0.5):
                self.update_state(sample_pos + translation_vec)
                # import pdb; pdb.set_trace()
                self.refine_target_box(sample_pos, sample_scales[int(scale_ind)], int(scale_ind), True)
                hard_negative = (flag == 'hard_negative')
                ...
                self.size = np.array([new_state[2], new_state[3]])
                self.center_pos = np.array([new_state[0] + new_state[2] / 2.,
                                            new_state[1] + new_state[3] / 2.])
                return new_state, mx_score
            ...
            if RPN_score >= 0.8:
                new_pos = [target_pos[1], target_pos[0]]
                target_sz = [target_sz[1], target_sz[0]]

                # ATOM correction
                self.target_sz = torch.Tensor(target_sz)
                self.update_state(torch.Tensor(new_pos))
                self.refine_target_box(sample_pos, sample_scales[int(scale_ind)], int(scale_ind), True)
            ...
            # otherwise restart ATOM and using RPN results
            if (self.frame_num - 1) % self.params.train_skipping == 0:
                self.filter_optimizer.run(self.params.CG_iter)
    ...
    def refine_target_box(self, sample_pos, sample_scale, scale_ind, update_scale = True):
        ...
        # if len(keep_ind) <= 0: return
        if sum(keep_ind) <= 0: return
        ind = []
    ...
    def IoU_2(self, rect1, rect2):
        # overlap
        import numpy as np
        x1, y1, x2, y2 = rect1[0] - rect1[2]/2., \
                         rect1[1] - rect1[3]/2., \
                         rect1[0] + rect1[2]/2., \
                         rect1[1] + rect1[3]/2.

        tx1, ty1, tx2, ty2 = rect2[0] - rect2[2]/2.,\
                             rect2[1] - rect2[3]/2.,\
                             rect2[0] + rect2[2]/2., \
                             rect2[1] + rect2[3]/2.
        xx1 = np.maximum(tx1, x1)
    ...
    def optimize_boxes(self, iou_features, init_boxes):
        ...
        return output_boxes.view(-1, 4).cpu().data, outputs.detach().view(-1).cpu().data
