...
# from libs.core import TensorList
from libs.utils import TensorList
    ...
    def get_feature(self, im: torch.Tensor):
        ...
        if self.normalize_power is not None:
            for idx, feat in enumerate(feat_list):
                feat_list[idx] /= (torch.sum(feat_list[idx].abs().view(feat_list[idx].shape[0],1,1,-1)**self.normalize_power, dim=3, keepdim=True) /
                         (feat_list[idx].shape[1]*feat_list[idx].shape[2]*feat_list[idx].shape[3]) + 1e-10)**(1/self.normalize_power)
