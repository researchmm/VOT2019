...
from settings.datasets import VOTLT18Dataset, VOTLT19Dataset, OXUVADataset, OTBDataset, LaSOTDataset, VOTRGBD19RGBDataset
...
class EXP:
    ...
    def __init__(self):
        self.pairs = []
        # self.pairs = self.fstar_vltlt2018() + self.fstar_vltlt2019()
        # self.pairs = self.fstar_oxuva()
        # self.pairs = self.fstar_otb() + self.fstar_lasot() + self.fstar_rgbd()
        # self.pairs = self.fstar_rgbd()
        self.pairs = self.fstar_lasot()
    ...
    def far_rgbd(self):
        trackers = [Tracker('far_fusion', 'res50_512_TLVC', 'rgbd', i) for i in range(1)]
        dataset = VOTRGBD19RGBDataset()
        pair_list = [(tracker_info, seq) for tracker_info, seq in product(trackers, dataset)]
        return pair_list
