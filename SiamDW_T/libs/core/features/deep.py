from core.features.featurebase import FeatureBase, MultiFeatureBase
import torch
import torchvision
from core import TensorList
from core.evaluation.environment import env_settings
import os
from models import load_network
from models.models.backbone.resnet18_vggm import resnet18_vggmconv1
...
class AASResNet18(MultiFeatureBase):
    """
    args:
        output_layers: List of layers to output.
        net_path: Relative or absolute net path (default should be fine).
        use_gpu: Use GPU or CPU.
    """
    def __init__(self, output_layers=('layer3',), net_path='aas_iou', use_gpu=True, *args, **kwargs):
    ...
    def extract(self, im: torch.Tensor, flag: str):
        ...
        output_features = self.net.extract_features(Variable(im), self.feature_layers, flag)
        ...
        # with torch.no_grad():  # pytorch0.3.1
        if flag == "RGB":
            self.iounet_features = TensorList(self.rgb_bb_regressor.get_iou_feat(self.iounet_backbone_features))
        elif flag == "T":
            self.iounet_features = TensorList(self.t_bb_regressor.get_iou_feat(self.iounet_backbone_features))
        else:
...
class AASResNet50(MultiFeatureBase):
    """
    args:
        output_layers: List of layers to output.
        net_path: Relative or absolute net path (default should be fine).
