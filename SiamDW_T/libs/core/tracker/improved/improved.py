...
from core import dcf, fourier, TensorList, operation
from core.features.preprocessing import numpy_to_torch
from core.utils.plotting import show_tensor
from core.libs.optimization import GaussNewtonCG, ConjugateGradient, GradientDescentL2
from .optim import ConvProblem, FactorizedConvProblem
from core.features import augmentation
import numpy as np


class AAS(BaseTracker):
    ...
    def initialize(self, raw_im, image, state, flag, *args, **kwargs):
        # flag: RGB or T
        self.flag = flag

        # Initialize some stuff
        self.init_online = kwargs['init_online']
        self.frame_num = 1
        self.gt_flag = False
        ...
        self.pos = torch.Tensor([state[1], state[0]])  # [y,x]
        self.target_sz = torch.Tensor([state[3], state[2]])  # [h,w]

        # Set search area
        self.target_scale = 1.0
        # search_area = torch.prod(self.target_sz * self.params.search_area_scale).item()
        search_area = torch.prod(self.target_sz * self.params.search_area_scale) # for pytorch0.3
        ...
        if getattr(self.params, 'search_area_shape', 'square') == 'square':
            ...
            area = torch.prod(self.base_target_sz * self.params.search_area_scale)  # pytorch0.3
        ...
        if getattr(self.params, 'window_output', False):
            if getattr(self.params, 'use_clipped_window', False):
                ...
                self.output_window = dcf.hann2d(self.output_sz.long(), centered=False).cuda()  # pytorch0.3.1
    ...
    def pytorch4round(self, a):
        a[0] = round(a[0])
        a[1] = round(a[1]) #
        # a[0] = self.pytorch4round_single(a[0])
        # a[1] = self.pytorch4round_single(a[1])
    ...
    def init_optimization(self, train_x, init_y):
        ...
        if self.init_online:
            self.filter = TensorList(
                [torch.zeros(1, cdim, sz[0], sz[1]).cuda() for x, cdim, sz in
        ...
        if self.params.update_projection_matrix:
            ...
            if optimizer == 'GaussNewtonCG':
                self.joint_optimizer = GaussNewtonCG(self.joint_problem, joint_var.variable(),
        ...
        compressed_samples = self.project_sample(self.init_training_samples, self.projection_matrix.variable()).tensor()
    ...
    def track_getrandom(self, raw_im, image, gt):
        saved_im = image
        self.frame_num += 1
        ...
        self.im = im    # For debugging only
        self.gt_flag = True
        self.gt_center = torch.Tensor([gt[0], gt[1]])
        self.gt_sz = torch.Tensor([gt[2], gt[3]])

        # ------- LOCALIZATION ------- #

        # Get sample
        # sample_pos = self.pos.round()
        sample_pos = self.pytorch4round(self.pos.clone())
        ...
        mx_score, translation_vec, scale_ind, s, flag = self.localize_target(scores_raw)

        # return random bbox -- utilied by another domin
        ...
        iou_features, init_boxes = self.refine_target_box_getrandom(sample_pos, sample_scales[int(scale_ind)], scale_ind, gt, update_scale_flag)

        
        self.temp_vals['sample_pos'] = sample_pos
        self.temp_vals['translation_vec'] = translation_vec
        self.temp_vals['sample_scales'] = sample_scales
        self.temp_vals['scale_ind'] = scale_ind
        self.temp_vals['s'] = s
        self.temp_vals['scale_ind'] = scale_ind
        self.temp_vals['flag'] = flag
        self.temp_vals['mx_score'] = mx_score
        self.temp_vals['test_x'] = test_x

        return iou_features, init_boxes
    
    def track_final(self):
        if getattr(self.params, 'use_classifier', True):
            self.update_state(self.temp_vals['sample_pos'] + self.temp_vals['translation_vec'], None)

        # record position after classification *******

        if self.params.debug >= 2:
            show_tensor(self.temp_vals['s'][self.temp_vals['scale_ind'],...], 5, title='Max score = {:.2f}'.format(torch.max(self.temp_vals['s'][self.temp_vals['scale_ind'],...]).item()))


        # ------- UPDATE ------- #

        # Check flags and set learning rate if hard negative
        update_flag = self.temp_vals['flag'] not in ['not_found', 'uncertain']
        hard_negative = (self.temp_vals['flag'] == 'hard_negative')
        learning_rate = self.params.hard_negative_learning_rate if hard_negative else None

        if update_flag:
            # Get train sample
            train_x = TensorList([x[int(self.temp_vals['scale_ind']):int(self.temp_vals['scale_ind'] + 1), ...] for x in self.temp_vals['test_x']])
            # train_x = TensorList([x[self.temp_vals['scale_ind']:self.temp_vals['scale_ind']+1, ...] for x in self.temp_vals['test_x']])

            # Create label for sample
            train_y = self.get_label_function(self.temp_vals['sample_pos'], self.temp_vals['sample_scales'][int(self.temp_vals['scale_ind'])])
        ...
        if self.use_iou_net and self.temp_vals['flag'] != 'not_found':
            self.pos = self.pos_iounet.clone()

        # Return new state
        # new_state = torch.cat((self.pos[[1,0]] - (self.target_sz[[1,0]]-1)/2, self.target_sz[[1,0]]))
        new_state = torch.cat((self.pos[[1,0]], self.target_sz[[1,0]]))   # for release

        return new_state.tolist(), self.temp_vals['mx_score']
    ...
    def localize_target(self, scores_raw):
        ...
        return max_score, translation_vec, scale_ind, scores, None
    ...
    def localize_advanced(self, scores):
        ...
        if bool(float(max_score1 < self.params.target_not_found_threshold)): # pytorch0.3.1
            return max_score1, translation_vec1, scale_ind, scores, 'not_found'
        ...
        scores_masked = scores[int(scale_ind):int(scale_ind) + 1, ...].clone()  # pytorch0.3.1
        ...
        # target_disp2 = max_disp2 - self.output_sz // 2  # pytorch0.3.1
        translation_vec2 = target_disp2 * (self.img_support_sz / self.output_sz) * self.target_scale

        # Handle the different cases
        # if max_score2 > self.params.distractor_threshold * max_score1:
        if bool(float(max_score2 > self.params.distractor_threshold * max_score1)):  # pytorch0.3.1
            ...
            if disp_norm2 > disp_threshold and disp_norm1 < disp_threshold:
                return max_score1, translation_vec1, scale_ind, scores, 'hard_negative'
            if disp_norm2 < disp_threshold and disp_norm1 > disp_threshold:
                return max_score2, translation_vec2, scale_ind, scores, 'hard_negative'
            if disp_norm2 > disp_threshold and disp_norm1 > disp_threshold:
                return max_score1, translation_vec1, scale_ind, scores, 'uncertain'

            # If also the distractor is close, return with highest score
            return max_score1, translation_vec1, scale_ind, scores, 'uncertain'

        # if max_score2 > self.params.hard_negative_threshold * max_score1 and max_score2 > self.params.target_not_found_threshold:
        if bool(float(max_score2 > self.params.hard_negative_threshold * max_score1)) and bool(float(max_score2 > self.params.target_not_found_threshold)):
            return max_score1, translation_vec1, scale_ind, scores, 'hard_negative'

        return max_score1, translation_vec1, scale_ind, scores, None

    def extract_sample(self, im: torch.Tensor, pos: torch.Tensor, scales, sz: torch.Tensor):
        return self.params.features.extract(im, pos, scales, sz, self.flag)
    ...
    def init_learning(self):
        ...
        self.feature_window = TensorList([dcf.hann2d(sz).cuda() for sz in self.feature_sz])  # pytorch0.3.1
    ...
    def generate_init_samples(self, im: torch.Tensor) -> TensorList:
        ...
        init_samples = self.params.features.extract_transformed(im, self.pos, self.target_scale, aug_expansion_sz, self.transforms, self.flag)
        ...
        if self.params.use_projection_matrix:
            ...
            if proj_init_method == 'pca':
                ...
                if self.init_online:
                    ...
                    #         temp = None
                    #     else:
                    #         temp = torch.zeros(cdim, ex.shape[1], 1, 1).normal_(0, 1 / math.sqrt(
                    #         ex.shape[1])).cuda()
                    ...
                    #      zip(x, self.compressed_dim)])  # pytorch0.3.1
        else:
    ...
    def init_label_function(self, train_x):
        ...
        # Center pos in normalized coords
        # target_center_norm = (self.pos - self.pos.round()) / (self.target_scale * self.img_support_sz)
        target_center_norm = (self.pos - self.pytorch4round(self.pos.clone())) / (self.target_scale * self.img_support_sz)   # pytorch0.4 round
    ...
    def get_iounet_box(self, pos, sz, sample_pos, sample_scale):
        ...
        if self.gt_flag:
            self.gt_center = (self.gt_center - sample_pos) / sample_scale + (self.iou_img_sample_sz - 1) / 2
            self.gt_sz = self.gt_sz / sample_scale
        target_ul = box_center - (box_sz - 1) / 2
        # return torch.cat([target_ul.flip((0,)), box_sz.flip((0,))])
        return torch.cat([self.flip(target_ul, 0), self.flip(box_sz, 0)]) # pytorch0.3.1


    def init_iou_net(self):
        # Setup IoU net -- zzp fix
        if self.flag == "RGB":
            self.iou_predictor = self.params.features.get_unique_attribute('rgb_bb_regressor')
        elif self.flag == "T":
            self.iou_predictor = self.params.features.get_unique_attribute('t_bb_regressor')
        else:
            raise ValueError("No suacj flag")

        self.iou_guess = self.params.features.get_unique_attribute('iou_guess')  # zzp -- get final result


        # self.iou_predictor = self.params.features.get_unique_attribute('iou_guess')
        ...
        self.iou_target_box = self.get_iounet_box(self.pos, self.target_sz, self.pytorch4round(self.pos.clone()), self.target_scale)   # pytorch0.4
        ...
        # target_boxes = torch.cat(target_boxes.view(1,4), 0).to(self.params.device)  # pytorch0.3.1
    ...
    # zzp -- return bbox utilized by anothe domin
    def refine_target_box_getrandom(self, sample_pos, sample_scale, scale_ind, gt, update_scale = True):
        ...
        iou_features = TensorList([x[int(scale_ind):int(scale_ind) + 1, ...] for x in iou_features])  # pytorch0.3.1
        ...
        if self.params.num_init_random_boxes > 0:
            ...
            square_box_sz = math.sqrt(init_box[2:].prod())  # pytorch0.3.1
            ...
            new_sz = (init_box[2:] + rand_bb[:, 2:]).clamp(min=minimal_edge_size)  # pytorch0.3.1
    ...
    # def refine_target_box_final(self, iou_features, init_boxes, gt=[0, 0, 0, 0]):
    #     output_boxes, output_iou = self.optimize_boxes(iou_features, init_boxes, gt)
    #
    #     return output_boxes, output_iou

    def refine_target_box_final(self, rgb_iou_features, t_iou_features,init_boxes, external_tracker, gt=[0, 0, 0, 0]):
        # for share head model
        output_boxes, output_iou = self.optimize_boxes(rgb_iou_features, t_iou_features, init_boxes, gt, external_tracker)

        return output_boxes, output_iou


    def refine_target_box_merge(self, output_boxes, output_iou):
    # Remove weird boxes with extreme aspect ratios
    #     output_boxes[:, 2:].clamp_(1)
        try:
            output_boxes[:, 2:].clamp_(min = 1)
        except:
            output_boxes = output_boxes.data
            output_boxes[:, 2:].clamp_(min=1)
        ...
        # # debug: use the first one to compare with 0.4
        # inds = 0
        # predicted_box = output_boxes[inds, :]
        # predicted_iou = output_iou.view(-1, 1)[inds, :]
        ...
        new_pos = self.flip(new_pos, 0) * self.temp_vals['sample_scales'] + self.temp_vals['sample_pos']
        new_target_sz = self.flip(predicted_box[2:], 0) * self.temp_vals['sample_scales']
    ...
    def optimize_boxes(self, rgb_iou_features, t_iou_features, init_boxes, gt, external_tracker):
        # Optimize iounet boxes
        output_boxes = Variable(init_boxes.view(1, -1, 4).cuda())  #pytorch0.3.1
        ...
        for i_ in range(self.params.box_refinement_iter):
            ...
            rgb_features, self.batch_size, self.num_proposals_per_batch = self.iou_predictor.predict_iou(self.target_feat, rgb_iou_features, bb_init)
            t_features, _, _ = external_tracker.iou_predictor.predict_iou(external_tracker.target_feat, t_iou_features, bb_init)


            outputs = self.iou_guess(rgb_features, t_features, self.batch_size, self.num_proposals_per_batch, 1)
