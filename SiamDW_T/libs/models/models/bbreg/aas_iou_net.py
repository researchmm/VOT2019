...
from models.models.layers.blocks import LinearBlock
from models.external.PreciseRoIPooling.pytorch.prroi_pool import PrRoIPool2D
...
class RGBIoUNet(nn.Module):
    ...
    def __init__(self, input_dim=(128,256), pred_input_dim=(256,256), pred_inter_dim=(256,256)):
        ...
        #         # nn.init.kaiming_normal(m.weight.data, mode='fan_in')   # for pytorch0.3 zzp
    ...
    def forward(self, feat1, feat2, bb1, proposals2):
        ...
        fc34_rt_cat, batch_size, num_proposals_per_batch = self.predict_iou(filter, iou_feat, proposals2)
        # return

        return fc34_rt_cat, batch_size, num_proposals_per_batch, num_test_images
    ...
    def predict_iou(self, filter, feat2, proposals):
        ...
        # batch_index = torch.Tensor([x for x in range(batch_size)]).view(batch_size, 1).cuda()  # pytorch0.3.1
        batch_index = Variable(torch.Tensor([x for x in range(batch_size)]).view(batch_size, 1).cuda())
        ...
        # roi2 = torch.cat((batch_index.view(batch_size, -1, 1).expand(-1, num_proposals_per_batch, -1),
        #                   proposals_xyxy), dim=2)
        # roi2 = roi2.view(-1, 5).to(proposals_xyxy.device) # pytorch0.3.1
        roi2 = roi2.view(-1, 5).cuda()
...
class TIoUNet(nn.Module):
    ...
    def predict_iou(self, filter, feat2, proposals):
        ...
        # Add batch_index to rois
        # batch_index = torch.Tensor([x for x in range(batch_size)]).view(batch_size, 1).cuda()
        batch_index = Variable(torch.Tensor([x for x in range(batch_size)]).view(batch_size, 1).cuda())
        # Push the different rois for the same image along the batch dimension
        num_proposals_per_batch = proposals.shape[1]
...
class iou_predictor(nn.Module):
    def __init__(self,  pred_inter_dim=(256,256)):
        super().__init__()
        # TODO: add more fc: since 1024 --> 1 is too big
        self.predictor = nn.Linear((pred_inter_dim[0] + pred_inter_dim[1]) // 2, 1, bias=True)
        self.fc1 = nn.Linear((pred_inter_dim[0] + pred_inter_dim[1]) * 1, (pred_inter_dim[0] + pred_inter_dim[1]) // 2, bias=True)
        self.fc2 = nn.Linear((pred_inter_dim[0] + pred_inter_dim[1]) * 1, (pred_inter_dim[0] + pred_inter_dim[1]) // 2, bias=True)

    def forward(self, feature_rgb, feature_t, batch_size, num_proposals_per_batch, num_test_images):
        feature = self.fc1(feature_rgb) * self.fc2(feature_t)
        iou_pred = self.predictor(feature).view(batch_size, num_proposals_per_batch)

        return iou_pred.view(num_test_images, batch_size, -1)
...
#                     m.bias.data.zero_()
#
#     def forward(self, rgb_feat1, rgb_feat2, t_feat1, t_feat2, bb1, proposals2):
#         assert rgb_feat1[0].dim() == 5, 'Expect 5  dimensional feat1'
#         assert t_feat1[0].dim() == 5, 'Expect 5  dimensional feat1'
#
#         num_test_images = rgb_feat2[0].shape[0]
#         batch_size = rgb_feat2[0].shape[1]
#
#         # Extract first train sample
#         rgb_feat1 = [f[0,...] for f in rgb_feat1]
#         t_feat1 = [f[0,...] for f in rgb_feat1]
#         bb1 = bb1[0,...]
#
#         # Get modulation vector
#         rgb_filter = self.get_filter(rgb_feat1, bb1)
#         t_filter = self.get_filter(t_feat1, bb1)
#
#         rgb_feat2 = [f.view(batch_size * num_test_images, f.shape[2], f.shape[3], f.shape[4]) for f in rgb_feat2]
#         t_feat2 = [f.view(batch_size * num_test_images, f.shape[2], f.shape[3], f.shape[4]) for f in t_feat2]
#         rgb_iou_feat = self.get_iou_feat(rgb_feat2)
#         t_iou_feat = self.get_iou_feat(t_feat2)
#         iou_feat = torch.cat(rgb_iou_feat, t_iou_feat)   # TODO: CHECK SIZE
#
#         rgb_filter = [f.view(1, batch_size, -1).repeat(num_test_images, 1, 1).view(batch_size*num_test_images, -1) for f in rgb_filter]
#         t_filter = [f.view(1, batch_size, -1).repeat(num_test_images, 1, 1).view(batch_size*num_test_images, -1) for f in t_filter]
#         filter = torch.cat(rgb_filter, t_filter)         # TODO: CHECK SIZE
#
#         proposals2 = proposals2.view(batch_size*num_test_images, -1, 4)
