...
def resnet50(output_layers=None, pretrained_path='pretrain/ResNet50.pth', ):
    ...
    if pretrained_path and os.path.exists(pretrained_path):
        ...
        # pretrained_dict.update({'conv1_weight':new_conv1_weight})
        # pretrained_dict['conv1_weight'] = new_conv1_weight
        new_dict = OrderedDict()
        # for key in pretrained_dict.keys():
        #     if key == 'conv1.weight':
        #         conv1_weight = pretrained_dict[key].contiguous()
        #         new_conv1_weight = torch.cat((conv1_weight, conv1_weight), dim=1)
        #         new_dict[key] = new_conv1_weight
        #     else:
        #         new_dict[key] = pretrained_dict[key]
        for key in pretrained_dict['net'].keys():
            if key.startswith('feature_extractor'):
                new_dict[key.split('feature_extractor.')[-1]] = pretrained_dict['net'][key]
            else:
                new_dict[key] = pretrained_dict['net'][key]

        # for key in new_dict.keys():
        # print("key: {} shape: {}".format(key, new_dict[key].shape))
        load_pretrain(model, new_dict)
    else:
        print('attention: no imagenet pretrain, testing?')
    return model